...
@app.route("/ngo_reg2",methods =["GET", "POST"])
def ngo_reg2():
    ...
    sql = "select count(*) from ngo where userid=' " + uid + " ' "
    cursor.execute(sql)
    res = cursor.fetchone()[0]
    #print("number of",res)
    if res > 0:
        return render_template("ngo_reg.html", msg="already exists..!")
    else:
        qry = "insert into ngo(name,userid,password,email,number) values  (%s,%s,%s,%s,%s)"
        values=(name,uid,pswd,email,mno)
        cursor.execute(qry,values)
        database.commit()
        return render_template("ngo.html",msg="added")
    return ""
...
@app.route("/ngo_reg")
def ngo_reg():
    return render_template("ngo_reg.html")
...
def ngologin():
        ...
        database = DBConnection.getConnection()
        cursor = database.cursor()
        sql = "select count(*) from ngo where userid='" + uid + "' and password='" + pwd + "'"
        cursor.execute(sql)
        res = cursor.fetchone()[0]
        if res > 0:
            session['uid'] = uid
            qry = "select * from ngo where userid= '" + uid + " ' "
            cursor.execute(qry)
            val = cursor.fetchall()
            for values in val:
                name = values[0]
                #print(name)
            return render_template("ngohome.html",name=name)
        else:
...
def uupload_photo2():
    try:
        ...
        age=age[1:-1].split("-")
        print("age::",age)
        if int(age[0])<18 and int(age[1])<18 :
            ...
            values = (name,number,adrs, new_image,gender,result[1],'wait')
            cursor.execute(query, values)
            database.commit()
            return render_template("user_upload_photo.html", message="Photo Uploaded Successfully..!")
        else:
            return render_template("user_upload_photo.html",msg2="No..!")
